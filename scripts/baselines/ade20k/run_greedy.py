"""
scripts/baselines/ade20k/run_greedy.py

Run a greedy episode and print per-step diagnostics.

Usage:
    python scripts/baselines/ade20k/run_greedy.py
    python scripts/baselines/ade20k/run_greedy.py \
        --episodes 10 --seed 42 --dataset datasets/ADE20k --verbose
    python scripts/baselines/ade20k/run_greedy.py \
        --miou --episodes 5 --t 5 --k 10 --dataset datasets/ADE20k
    python scripts/baselines/ade20k/run_greedy.py --miou-mode accumulator --all --split validation --t 5 --k 10
    python scripts/baselines/ade20k/run_greedy.py \
...
from canvit_rl.ade20k.greedy import run_greedy_batch
...
def _make_comet_experiment(args: argparse.Namespace):
    ...
    try:
        # Import Comet only when logging is enabled; importing it during --help
        # can load IPython integrations that crash in some Python stacks.
