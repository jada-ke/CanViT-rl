...
def format_recommendation(traces: list[StateTrace], *, rank_window: int) -> list[str]:
    ...
    lines = [
        ...
        "## Recommendation",
        "",
        "Use `--reconstruction-norm-state` as the main clean state candidate. Drop `--teacher-reconstruction-error-state` from the main method because it needs teacher targets and is only marginally better than reconstruction norm in this screen.",
        "",
        "The current data suggests the useful signal is the target-free reconstruction-norm aux map. Explicit viewpoint history alone is weak in this single-seed train-curve screen, but history may still help when paired with reconstruction norm because `canvas_reconstructionnorm` beats `canvas_no_hist_reconstructionnorm`.",
        "",
        "Recommended next comparisons:",
        "",
        "| Priority | State | Why |",
        "|---:|---|---|",
        "| 1 | `--reconstruction-norm-state` | Best clean candidate; strong endpoint and best AUC. |",
        "| 2 | `--disable-viewpoint-history-state` | Canvas-only baseline; tests whether explicit coordinate history matters. |",
        "| 3 | `--reconstruction-norm-state --disable-viewpoint-history-state` | Isolates whether viewpoint history adds value once reconstruction norm is present. |",
        "| Reference | default viewpoint-history | Baseline only; weak by this screen. |",
        "",
        "## Endpoint Ranking",
        "",
        "| Rank | State Trace | State Label | Last-Window Mean | Gap vs Best | Relative Gap | First | Improvement |",
        "|---:|---|---|---:|---:|---:|---:|---:|",
    ]
    ...
    lines.extend(
        [
            ...
            "## Robustness Table",
            "",
            "| State Trace | Last-5 Rank | AUC Rank | Smooth Rank | Last-5 | Last-10 | Last-20 | AUC | Smoothness | Best |",
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for trace in by_last:
        lines.append(
            "| "
            f"`{trace.name}` | {last_rank[trace.name]} | {auc_rank[trace.name]} | "
    ...
    lines.extend(
        [
            ...
            "- `reconstruction_norm` with viewpoint history is stronger than no-history reconstruction norm, so history may help once the aux map is present.",
            "- Plain viewpoint-history is weak here; the current CanViT canvas already carries implicit history, so explicit coordinate history alone is not clearly useful.",
            "- Detail debt and cos-prev do not improve this single-seed train-curve screen.",
        ]
    )
    return lines
