...
Unlike scripts/baselines/ade20k/run_entropy_c2f.py, this script uses an mIoUAccumulator per
timestep, so reported mIoU is accumulated over the dataset rather than averaged
from per-image mIoUs.

Usage:
    python scripts/evaluation/ade20k/eval_entropy_c2f_miou.py
    python scripts/evaluation/ade20k/eval_entropy_c2f_miou.py --batch-size 8 --t 21
    python scripts/evaluation/ade20k/eval_entropy_c2f_miou.py --t 21 --batch-size 8 --max-batches 2
"""
